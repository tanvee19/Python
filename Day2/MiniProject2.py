# -*- coding: utf-8 -*-
"""
Created on Wed May  8 19:24:53 2019
...
while True:
    ...
    if new_item == 'SHOW':
        ...
        continue
    if new_item == 'HELP':
        print("If you want to add something in the list then add by typing the name of the item")
        print("If you are ready with the final list type DONE")
        print("If you want to see the list of the chosen items then type SHOW")
        continue
...
for eitem in new_item:
    if new_item == 'DONE':
           break
            ...
            # add new items to our list
            if new_item == "ADD":
                index = int(input("Enbter the index value at which the must be added"))
                for i in range(len(shopping_list)):
                    if(i == index):
                        add(i)
            if new_item == 'REMOVE':
                index1 = int(input("Enbter the index value at which the must be added"))
                for i in range(len(shopping_list)):
                    if(i == index1):
                        tem = shopping_list[i]
                        remove(tem)
